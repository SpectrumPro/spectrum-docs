...
6. **Always document `_init`**, even if it takes no arguments. Place it as the first entry under `## Public API`. Use `*Args:* null` if there are no parameters. **Exception: if the class defines `_static_init` instead of `_init`, it is a static class — do not add a `_init` entry at all.**
...
11. **Document all public member variables and static variables** under `## Member Variables`, each as a `###` entry with `*DataType:*`, `*Default:*` (if explicitly set in source), and a description. Omit `*Default:*` if no default is set. Place this section directly before `## Public API`.
12. **Document all `_settings` registrations** under `## SettingsManager Entries` as a single table. Identify the EntryType from the registration method: `register_setting` → `SETTING`, `register_control` → `CONTROL`, `register_status` → `STATUS`.
