...
- **`GlobalBaseClass`** — the structural interface all GBC-compatible classes must implement. Defines the required signals, methods, and serialization contract.
- **`Data`** — the canonical type enum used throughout the engine to describe and identify data values.
- **`ChildManager`** — a utility class for managing parent-child object relationships through a unified callable-based interface.
